...
下一章我们会学习 **字符串处理**——Java 中最常用的数据类型。你会学到 String 的不可变性、字符串常量池、StringBuilder 高效拼接、字符串常用方法和格式化。

---

## 本章小结

本章学习了 Java 数组的声明、初始化、遍历和常用操作。数组是固定长度的同类型元素集合，使用 `Arrays` 工具类可以方便地进行排序、查找、复制等操作。接下来我们将学习字符串处理。
